...
export function SiteHeader() {
  ...
  return (
    <>
      <header
        className={cn(
          "fixed inset-x-0 top-0 z-50 flex h-20 items-center justify-between px-5 transition-colors md:h-24 md:px-10",
          isHome
            ? "text-[var(--color-ink)]"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="grid size-11 place-items-center rounded-full border border-current/35 transition-colors hover:bg-white/30"
            aria-label="背景音乐暂未启用"
            title="背景音乐暂未启用"
          >
            <MusicNotesIcon size={20} weight="light" aria-hidden />
          </button>
          <button
            type="button"
            className="grid size-11 place-items-center rounded-full transition-colors hover:bg-white/30"
            onClick={() => setOpen(true)}
            aria-label="打开菜单"
            aria-expanded={open}
          >
            <ListIcon size={28} weight="light" aria-hidden />
          </button>
        </div>
      </header>
